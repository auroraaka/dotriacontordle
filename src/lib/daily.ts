...
function getEasternTimeComponents(date: Date): {
  year: number;
  month: number;
  day: number;
  hour: number;
...
function easternTimeToUTC(
  ...
  for (let utcHour = 11; utcHour <= 14; utcHour++) {
    ...
    if (
      eastern.year === targetEastern.year &&
...
function getNext8AMEastern(): Date {
  ...
  if (today8AM.getTime() <= now.getTime()) {
    const tomorrowUTC = new Date(
      Date.UTC(eastern.year, eastern.month - 1, eastern.day + 1, 12, 0, 0)
    );
    const tomorrowEastern = getEasternTimeComponents(tomorrowUTC);
    return easternTimeToUTC(
      tomorrowEastern.year,
      tomorrowEastern.month,
      tomorrowEastern.day,
      8,
      0,
      0
...
function getCurrentDayStartEastern(): Date {
  ...
  if (today8AM.getTime() > now.getTime()) {
    ...
    return easternTimeToUTC(
      yesterdayEastern.year,
      yesterdayEastern.month,
      yesterdayEastern.day,
      8,
      0,
      0
    );
...
export function getDailyAnswers(
  dailyNumber: number,
  config: Pick<GameConfig, 'boardCount' | 'wordLength'> = DEFAULT_GAME_CONFIG
): string[] {
